...
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 128)
) C
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets codesets on ((de.drug_concept_id = codesets.concept_id and codesets.codeset_id = 162))
) C
