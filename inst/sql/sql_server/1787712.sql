...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1123630,40170911)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1123630,40170911)
  and c.invalid_reason is null
...
FROM (
	...
	JOIN #Codesets cs on cs.codeset_id = 171 AND de.drug_concept_id = cs.concept_id
	...
	FROM @cdm_database_schema.DRUG_EXPOSURE de
	JOIN ctePersons p on de.person_id = p.person_id
	JOIN #Codesets cs on cs.codeset_id = 171 AND de.drug_source_concept_id = cs.concept_id
) E
;
